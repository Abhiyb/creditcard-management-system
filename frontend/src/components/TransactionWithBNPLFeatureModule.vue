  ...
  <div class="min-h-screen bg-gray-50 p-4 md:p-8 font-sans">
    <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
      ...
      <div class="bg-gradient-to-r from-blue-700 to-indigo-700 text-white p-6 flex justify-between items-center">
        ...
        <button @click="$router.push('/dashboard')" class="flex items-center gap-2 px-5 py-2.5 bg-white text-blue-700 rounded-2xl font-medium hover:bg-gray-100 transition-all active:scale-95">
          ← Dashboard
        </button>
      </div>
      ...
      <div class="p-8">
        ...
        <div v-if="currentStep === 6" class="space-y-8">
          ...
          <div v-if="Object.keys(groupedDisplayedInstallments).length === 0" class="text-center py-16 bg-gray-50 rounded-3xl">
            <p class="text-gray-400">No installments found yet</p>
          </div>
          ...
          <button @click="currentStep = 0" class="w-full py-4 border border-gray-300 rounded-3xl text-gray-700 font-medium">Back to New Payment</button>
        </div>
      </div>
    </div>
  </div>
...
const groupedDisplayedInstallments = computed(() => {
  ...
  filteredDisplayedInstallments.value.forEach(i => {
    const tid = i.transactionId || 'Unknown'
...
const goToStep = (index) => { if (index <= currentStep.value) currentStep.value = index }
