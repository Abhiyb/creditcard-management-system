  ...
  <div class="min-h-screen bg-gray-50 p-4 md:p-8">
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
      ...
      <div class="bg-primary text-white p-6 flex justify-between items-center">
        ...
        <button @click="router.push('/dashboard')" class="px-4 py-2 bg-white text-primary rounded-md hover:bg-gray-100 transition-colors" aria-label="Back to Dashboard">
          Back to Dashboard
        </button>
      </div>
      ...
      <div class="px-6 pt-6">
        <div class="flex items-center justify-between mb-8" role="navigation" aria-label="BNPL process steps">
          <div v-for="(step, index) in steps" :key="index" class="flex flex-col items-center flex-1">
            <div :class="`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${currentStep >= index ? 'bg-primary text-white' : 'bg-gray-200 text-gray-500'}`" :aria-current="currentStep === index ? 'step' : undefined">
              {{ index + 1 }}
            </div>
      ...
      <div class="p-6">
        ...
        <div v-if="currentStep === 0" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2 md:col-span-2">
              ...
              <input v-model="transaction.cardNumber" type="text" maxlength="19" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary" placeholder="1234 5678 9012 3456" required />
            </div>
            <div class="space-y-2">
              <label class="block text-sm font-medium text-gray-700">CVV</label>
              <input v-model="transaction.cvv" type="password" maxlength="4" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary" placeholder="123 or 1234" required />
            </div>
            <div class="space-y-2">
              <label class="block text-sm font-medium text-gray-700">Expiry Date (MM/YY)</label>
              <div class="flex gap-4">
                <input v-model="transaction.expiryMonth" type="text" maxlength="2" placeholder="MM" class="w-20 px-4 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary" required />
                <span class="self-center text-gray-500">/</span>
                <input v-model="transaction.expiryYear" type="text" maxlength="2" placeholder="YY" class="w-20 px-4 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary" required />
              </div>
            </div>
            <div class="space-y-2">
              <label class="block text-sm font-medium text-gray-700">Amount (₹)</label>
              <input v-model.number="transaction.amount" type="number" step="0.01" min="1" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary" placeholder="Enter amount" required />
            </div>
            <div class="space-y-2">
          ...
          <div class="pt-4 flex justify-between">
            <button @click="viewTransactionHistory" class="px-6 py-3 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 transition-colors">
              View Transaction History
            </button>
            <button @click="checkEligibility" class="px-6 py-3 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors" :disabled="loading || !isFormValid">
        ...
        <div v-if="currentStep === 1" class="space-y-6">
          <div v-if="eligibilityResult.eligible" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div @click="selectPaymentMethod('full')" :class="`border rounded-lg p-4 cursor-pointer transition-all ${selectedPaymentMethod === 'full' ? 'border-primary bg-primary-50' : 'border-gray-200 hover:border-primary'}`" role="button">
              ...
              <div @click="selectPaymentMethod('bnpl')" :class="`border rounded-lg p-4 cursor-pointer transition-all ${selectedPaymentMethod === 'bnpl' ? 'border-primary bg-primary-50' : 'border-gray-200 hover:border-primary'}`" role="button">
                <div class="flex items-center">
            ...
            <div v-if="selectedPaymentMethod === 'bnpl'" class="mt-6 space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div v-for="plan in installmentPlans" :key="plan.months" @click="selectInstallmentPlan(plan)" :class="`border rounded-lg p-4 cursor-pointer transition-all ${selectedPlan && selectedPlan.months === plan.months ? 'border-primary bg-primary-50' : 'border-gray-200 hover:border-primary'}`" role="button">
            ...
            <div class="pt-4 flex justify-between">
              ...
              <button @click="proceedToConfirmation" class="px-6 py-3 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors" :disabled="!canProceed || loading">
        ...
        <div v-if="currentStep === 2" class="space-y-6">
          ...
          <div class="pt-4 flex justify-between">
            ...
            <button @click="confirmTransaction" class="px-6 py-3 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors" :disabled="loading">
              <span v-if="loading">Processing...</span>
        ...
        <div v-if="currentStep === 3" class="space-y-6 text-center py-8">
          ...
          <div class="pt-6 flex flex-col md:flex-row justify-center space-y-3 md:space-y-0 md:space-x-3">
            <button v-if="selectedPaymentMethod === 'bnpl'" @click="goToInstallments" class="px-6 py-3 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors" :disabled="loading">
            ...
            <div class="flex justify-between items-center flex-wrap gap-4">
              <div class="space-y-1">
                ...
                <div class="flex space-x-2">
                  <input v-model="searchTransactionId" type="text" placeholder="Enter Transaction ID" class="px-3 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary w-64" />
                  <button @click="fetchInstallments" class="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors" :disabled="loading || !searchTransactionId">
                    <span v-if="loading">Loading...</span>
              ...
              <div class="flex space-x-2">
                <button @click="viewTransactionHistory" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 transition-colors">View Transactions</button>
                <button @click="resetForm" class="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors">New Payment</button>
              </div>
            </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      <tr v-for="installment in group" :key="installment.id">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button v-if="!installment.isPaid" @click="openPaymentModal(installment)" class="px-3 py-1 bg-primary text-white text-sm rounded-md hover:bg-primary-dark transition-colors" :disabled="paymentLoading">
                            Pay Now
                          </button>
                        </td>
                      </tr>
          ...
          <div class="space-y-4">
            <div class="flex justify-between items-center flex-wrap gap-4">
              <div class="space-y-1 flex-1 min-w-[280px]">
                ...
                <div class="flex space-x-2">
                  <input v-model="searchCardId" type="text" placeholder="Enter Card ID (numeric)" class="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary" />
                  <button @click="fetchTransactions" class="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors" :disabled="loading || !searchCardId">
                    <span v-if="loading">Loading...</span>
                    <span v-else>Search</span>
                  </button>
                </div>
              ...
              <div class="flex space-x-2">
                <button @click="goToInstallments" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 transition-colors">View Installments</button>
                <button @click="resetForm" class="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-dark transition-colors">New Payment</button>
              </div>
            </div>
            ...
            <div class="flex flex-wrap gap-4 items-end">
              <div class="space-y-1">
                ...
                <input v-model="dateFilter.from" type="date" class="px-3 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary" />
              </div>
              <div class="space-y-1">
                <label class="block text-sm font-medium text-gray-700">To Date</label>
                <input v-model="dateFilter.to" type="date" class="px-3 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary" />
              </div>
              <button @click="applyDateFilter" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-colors">Apply Filter</button>
              <button @click="clearDateFilter" class="px-4 py-2 text-gray-500 rounded-md hover:text-gray-700 transition-colors">Clear</button>
            </div>
            ...
            <div v-else class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="transaction in filteredTransactions" :key="transaction.id">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <span :class="`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${transaction.isBNPL ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button v-if="transaction.isBNPL" @click="viewTransactionInstallments(transaction.id)" class="text-primary hover:text-primary-dark">
                        View Installments
                      </button>
                    </td>
                  </tr>
              ...
              <!-- Pagination -->
              <div v-if="filteredTransactions.length > 0" class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6 mt-4">
                <div class="flex-1 flex justify-between sm:hidden">
                  <button @click="prevPage" :disabled="currentPage === 1" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50">Previous</button>
                  <button @click="nextPage" :disabled="currentPage === totalPages" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50">Next</button>
                </div>
                <div class="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
                  <div>
                    <p class="text-sm text-gray-700">
                      Showing <span class="font-medium">{{ (currentPage - 1) * pageSize + 1 }}</span> to
                      <span class="font-medium">{{ Math.min(currentPage * pageSize, filteredTransactions.length) }}</span> of
                  ...
                  <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                    ...
                    <button v-for="page in paginationRange" :key="page" @click="goToPage(page)" :class="`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${currentPage === page ? 'z-10 bg-primary border-primary text-white' : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50'}`">
                      {{ page }}
                    </button>
      ...
      <div v-if="showPaymentModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden">
          <div class="bg-primary text-white px-6 py-4">
            <h3 class="text-lg font-semibold">Confirm Installment Payment</h3>
          </div>
          ...
          <div class="p-6 space-y-5">
            <div class="bg-gray-50 p-4 rounded-lg">
              ...
              <div class="flex justify-between mb-2">
                ...
                <span class="font-bold text-lg">₹{{ selectedInstallment?.amount?.toFixed(2) }}</span>
              </div>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">Enter CVV to proceed</label>
              <input
                v-model="paymentCvv"
                type="password"
                maxlength="4"
                placeholder="123 or 1234"
                class="w-full px-4 py-3 border border-gray-300 rounded-lg text-center text-xl tracking-widest focus:ring-primary focus:border-primary focus:outline-none"
                autocomplete="off"
              />
            </div>
          ...
          <div class="px-6 py-4 border-t flex gap-4">
            <button @click="cancelPayment" class="flex-1 py-3 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors" :disabled="paymentLoading">
              Cancel
            </button>
            <button
              @click="confirmInstallmentPayment"
              class="flex-1 py-3 bg-primary text-white rounded-lg font-medium hover:bg-primary-dark transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
...
const lastBnplTransactionId = ref(localStorage.getItem('lastBnplTransactionId') || '')
...
const filteredTransactions = computed(() => {
  ...
  if (transactionFilter.value !== 'all') {
    if (transactionFilter.value === 'full') filtered = filtered.filter(t => !t.isBNPL)
    if (transactionFilter.value === 'bnpl') filtered = filtered.filter(t => t.isBNPL)
  }

  // Simple date filter (you can improve with proper date parsing)
...
const checkEligibility = async () => {
  ...
  try {
    ...
    // Save last used card
    localStorage.setItem('lastUsedCardId', payload.cardNumber)
  ...
  try {
    ...
    if (savedTransaction.id) {
      if (transaction.value.isBNPL || savedTransaction.isBNPL) {
        localStorage.setItem('lastBnplTransactionId', savedTransaction.id.toString())
      }
      localStorage.setItem('lastUsedCardId', payload.cardNumber)
    }
...
const confirmInstallmentPayment = async () => {
  if (paymentCvv.value.length < 3) {
    paymentError.value = 'CVV must be 3 or 4 digits'
    return
  }

  const confirmed = confirm(
    `Are you sure you want to pay ₹${selectedInstallment.value.amount.toFixed(2)} ` +
    `for installment #${selectedInstallment.value.installmentNumber} now?`
  )
  ...
  const token = getAuthToken()
  if (!token) {
    paymentError.value = 'Session expired. Please login again.'
    paymentLoading.value = false
    return
  }
  ...
  try {
    ...
    const index = displayedInstallments.value.findIndex(i => i.id === updatedInstallment.id)
    if (index !== -1) {
      displayedInstallments.value[index] = updatedInstallment
    }

    showPaymentModal.value = false
    alert('Payment successful! Installment marked as Paid.')
  } catch (err) {
    paymentError.value = err.message || 'Could not process payment. Please try again.'
  } finally {
    paymentLoading.value = false
  }
}
...
const fetchTransactions = async () => {
  ...
  try {
    ...
    // Save for next time
    localStorage.setItem('lastUsedCardId', searchCardId.value)
  } catch (err) {
...
const viewTransactionHistory = () => {
  ...
  if (lastCard && !searchCardId.value) {
    searchCardId.value = lastCard
  }
  currentStep.value = 5
...
const resetForm = () => {
  ...
  errorMessage.value = ''
  transactions.value = []
  // Do NOT clear lastUsedCardId and lastBnplTransactionId – we want auto-fill
}
...
const applyDateFilter = () => { /* implement if needed */ }
...
onMounted(() => {
  const lastCard = localStorage.getItem('lastUsedCardId')
  if (lastCard) searchCardId.value = lastCard
})
</script>
